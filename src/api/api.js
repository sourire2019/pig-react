...
const displaypig = () => {
  return get('http://172.16.1.170:8080/getAllPig').then((res) => {
    return res;
  });
};

const addPig = (value) => {
  return post('http://172.16.1.170:8080/addpig', {
    data: value,
  });
};
...
// 显示猪舍信息
const pigsty = () => {
  return get('http://172.16.1.170:8080/pigHouseList');
};
...
const addPigsty = (value) => {
  return post('http://172.16.1.170:8080/addPighouse', {
    data: value,
  });
};
...
const pigstylist = () => {
  return get('http://172.16.1.170:8080/pigHouseIdList').then((resp) => {
    return resp;
  });
};
// mock
